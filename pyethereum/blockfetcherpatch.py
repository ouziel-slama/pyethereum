    ...
    for x in data:
        ...
        if tb.number == 1:
            ...
            for tb in sorted(collected_blocks, key=attrgetter('number')):
                print('writing', tb)
                fh.write(utils.encode_hex(tb.rlpdata) + '\n')  # LOG line
            sys.exit(0)
    # fetch more
    print("ASKING FOR MORE HASHES", utils.encode_hex(tb.hash), tb.number)
...
def _recv_Status(self, data):
    ...
    head_hash = data[3]
    print("head_hash", utils.encode_hex(head_hash))
...
def _recv_BlockHashes(self, data):
    ...
    # print [utils.encode_hex(x) for x in data]
